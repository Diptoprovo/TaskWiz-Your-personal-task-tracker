...
function ToDoList(){
    ...
    return(<>
        ...
        <div className="to-do-list font-sans  ">
            ...
            <h1 className=" text-center pt-10 font-bold text-4xl text-white"><lord-icon trigger="hover" src="/src/assets/logo.json"></lord-icon><span className="mx-2">TaskWiz</span></h1>
            ...
            <div className="flex flex-row justify-center">
                ...
                <button onClick={addTask} className="px-2"><lord-icon trigger="hover" src="/src/assets/add.json"></lord-icon></button>
            </div>
                ...
                <ol className="list flex flex-col">
                    {tasks.map((task, index) => 
                        <li key={index} className="element flex flex-row my-3 py-3 border rounded-2xl">
                            ...
                            <textarea id="task-text" className=" text-green-200 basis-10/12 text-center font-medium text-xl content-center bg-transparent px-5 outline-none" readOnly>
                            {task}
                            </textarea>
                            
                            <button onClick={() => taskDone(index)} className="px-2"><lord-icon trigger="hover" src="/src/assets/done.json"></lord-icon></button>
                            <button onClick={() => removeTask(index)} className="px-2"><lord-icon trigger="hover" src="/src/assets/del.json"></lord-icon></button>
                            <button onClick={() => moveUp(index)} className="px-2"><lord-icon trigger="hover" src="/src/assets/up.json"></lord-icon></button>
                            <button onClick={() => moveDown(index)} className="px-2"><lord-icon trigger="hover" src="/src/assets/down.json"></lord-icon></button>
                        </li>)}
                </ol>
            </div>
            <div className="mx-10 my-20 py-5 flex flex-col">
            <h3 className="text-2xl text-green-400 text-center cursor-pointer content-center w-fit"  onClick={showComplete}><lord-icon trigger="hover" src="/src/assets/show2.json"></lord-icon><span className="px-2">Recent tasks: </span></h3>
            
            {/* <button onClick={showComplete} className="px-2"><lord-icon trigger="hover" src="/src/assets/show.json">recent</lord-icon></button> */}
            
            <ul className="clist flex flex-col invisible" ref={clist}>
             {completed.map((task, index) => 
                    <li key={index} className="element flex flex-row my-3 py-3 border rounded-2xl">
                        <p className="  text-green-200 basis-11/12 text-center font-medium text-xl content-center">
                        {task}
                        </p>
                        <button onClick={() => removeComplete(index)} className="px-2 basis-1/12"><lord-icon trigger="hover" src="/src/assets/del.json"></lord-icon></button>
                    </li>)} 
            </ul>
            </div>
        </div>
